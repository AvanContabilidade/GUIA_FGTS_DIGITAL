...
parser = argparse.ArgumentParser(description="Automação com parâmetros")
parser.add_argument("--id", required=True, help="Id do processo")

args = parser.parse_args()

id = args.id

print (f"Agendamento {id} estpa sendo executado.")

requisicao = requests.get(
    'http://localhost:3100/parametros/automacoes/'+id
)
...
def advertencia_1(advertencia1, advertencia2, similaridade=0.8):
    try:
        # Verifica se a imagem está sendo analisada
...
def capturar_icone_inicial():
    bbox = (1195, 41, 1246, 60)  #mudar dependendo da tela
